...
class ExamplesDocumentsRepository implements RepositoryInterface
{
    ...
    public function all()
    {
        return $this->with('documents_type')->get();
    }
    ...
    public function delete($id)
    {
        if ($this->model->destroy($id)) {
            ...
            throw new Exception('Ошибка удаления образца');
        }
    }
    ...
    {
        if ($array->url) {
            $fileParams = File::delete($array->url);
        ...
        $url = Storage::disk('yandex')->put('/examples_documents', $array->file('file'));
